...
from config import (
    THRUSTER_PORT, CLAW_PORT, CAMERA_SERVO_PORT, BAUD_RATE,
...
class ROVState:
    ...
    thruster_port_connected:   bool = False
    claw_port_connected:       bool = False
    camera_servo_port_connected: bool = False
...
class ROVWorker(QThread):
    ...
    def __init__(self, parent=None):
        ...
        self.thruster_port     = SerialPort(THRUSTER_PORT,     BAUD_RATE, name="Thrusters")
        self.claw_port         = SerialPort(CLAW_PORT,         BAUD_RATE, name="Claw")
        self.camera_servo_port = SerialPort(CAMERA_SERVO_PORT, BAUD_RATE, name="CameraServo")
    ...
    def run(self):
        ...
        self._send_neutral()
        self.thruster_port.close()
        self.claw_port.close()
        self.camera_servo_port.close()
        pygame.quit()

    def stop(self):
        """Request the worker to exit cleanly. Call from GUI thread."""
        self._running = False
        self.wait()     


    def _poll(self, joystick) -> ROVState:
        """Read controller, write to serial, return a fresh ROVState."""
        ...
        if joystick is not None:
            ...
            if self._btn(joystick, BTN_SQUARE):
                ...
                # any other non-Square button clears estop
                if any(
        ...
        return ROVState(
            ...
            thruster_port_connected     = self.thruster_port.connected,
            claw_port_connected         = self.claw_port.connected,
            camera_servo_port_connected = self.camera_servo_port.connected,
    ...
    def _handle_claw(self, joystick):
        ...
        if claw_int != self._last_claw_sent:
            self.claw_port.send(f"claw:{claw_int}")
    ...
    def _handle_roll(self, joystick):
        ...
        if roll_int != self._last_roll_sent:
            self.claw_port.send(f"roll:{roll_int}")
    ...
    def _handle_tilt(self, joystick):
        ...
        if tilt_int != self._last_tilt_sent:
            self.camera_servo_port.send(f"tilt:{tilt_int}")
    ...
    def _send_neutral(self):
        cmd = "THR " + " ".join(str(THRUSTER_NEUTRAL) for _ in range(6))
        self.thruster_port.send(cmd)
        self._thrust_pwm = [THRUSTER_NEUTRAL] * 6

    @staticmethod
    def _axis(joystick, idx: int) -> float:
        try:
    ...
    @staticmethod
    def _btn(joystick, idx: int) -> bool:
        try:
...
class ROVController:
    ...
    def state_updated(self):
        """pyqtSignal — connect GUI slots here."""
